...
<div class="right_col" role="main" style="min-height: 3823px;">
    ...
    <div class="row">
        <div class="col-md-12 col-sm-12 ">
            <div class="x_panel">
                <div class="x_title">
                    <h2><?= $page == 'add' ? 'Tambah' : 'Edit' ?> <small>Dosen Kelas</small></h2>
                    <div class="clearfix"></div>
                </div>
                <div class="x_content">
                    <br>
                    <form method="post" action="<?= base_url('Admin/GuruKelas/process') ?>">
                        <?= form_hidden('id_GuruKelas', $row->id_kelas_detail) ?>
                        <div class="form-group">
                            <label for="tingkat">Tingkat GuruKelas<span class="required">*</span>
                            </label>
                            <select name="tingkat" id="tingkat" class="form-control <?= form_error('tingkat') != null ? 'border border-danger' : '' ?>">
                                <option value="">Silahkan pilih tingkat GuruKelas</option>
                        ...
                        <div class="form-group">
                            <label for="nama">Nama GuruKelas <span class="required">*</span></label>
                            <input type="text" id="nama" class="form-control <?= form_error('nama') != null ? 'border border-danger' : '' ?>" name="nama" value="<?= $row->nama == null ? set_value('nama') : $row->nama ?>" placeholder="Enter nama GuruKelas">
                            <?= form_error('nama') ?>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="guru_id[]" multiple="multiple" id="guru_id" class="form-control js-example-basic-multiple <?= form_error('guru_id') != null ? 'border border-danger' : '' ?>">
                                ...
                                <?php foreach ($guru as $result) : ?>
                                    <option value="<?= $result->id_guru; ?>"><?= $result->nama; ?> | <?= $result->status_ttq == "y" ? "Guru TTQ" : "Wali GuruKelas"; ?></option>
                                <?php endforeach; ?>
                            </select>
                            <?= form_error('guru_id') ?>
                        </div>
                        <?php if ($page == 'edit') : ?>
                            <div class="alert alert-info">
                                <strong>Nama Guru Sebelumnya</strong>
                                <br>
                                <ul>
                                    <?php $detail = $this->db->get_where('GuruKelas_detail', ['GuruKelas_id' => $row->id_GuruKelas])->result(); ?>
                                    <?php foreach ($detail as $result) : ?>
                                        <li><?= check_guru_lengkap($result->guru_id)->nama ?> | <?= check_guru_lengkap($result->guru_id)->status_ttq == "y" ? "Guru TTQ" : "Wali GuruKelas"; ?></li>
                                    <?php endforeach; ?>
                                </ul>
                            </div>
                        <?php endif; ?>
