...
class Siswa extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $roles = check_roles_crud('Admin/Siswa', $my_roles);
        ...
        $this->load->model(['Siswa/Siswa_model', 'Siswa/Server_Siswa_model', 'Auth/Users_model', 'KelasSiswa/KelasSiswa_model']);
    }
    ...
    public function process()
    {
        ...
        if (isset($_POST['add'])) {
            if ($this->form_validation->run() == false) {
                ...
                if ($insert > 0) {
                    sweetAlert2('success', 'Successfully', 'Berhasil tambah data Siswa', null, 'Admin/Siswa');
                } else {
                    sweetAlert2('success', 'Successfully', 'Berhasil tambah data Siswa', null, 'Admin/Siswa');
                }
            }
            ...
            if ($this->form_validation->run() == false) {
                ...
                $get = $this->Siswa_model->get($id)->row();
                ...
                $update = $this->Siswa_model->update($data, $id);
                if ($update > 0 || $updateUsers > 0) {
                    sweetAlert2('success', 'Successfully', 'Berhasil update data Siswa', null, 'Admin/Siswa');
                } else {
                    sweetAlert2('error', 'Failed', 'Gagal update data Siswa', null, 'Admin/Siswa');
                }
            }
        } else {
            show_404();
    ...
    public function delete()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            $id_Siswa = htmlspecialchars($this->input->post('id', true));
            $this->removeImage($id_Siswa);
            $row = $this->Siswa_model->get($id_Siswa)->row();
            $Siswa = $this->Siswa_model->delete($id_Siswa);
    ...
    public function deleteAllSiswa()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            $id_Siswa = $this->input->post('data_item');
            foreach ($id_Siswa as $index => $result) {
                $this->removeImage($result);
                $row = $this->Siswa_model->get($result)->row();
                $Siswa = $this->Siswa_model->delete($result);
                $this->db->delete('users', ['id_users' => $row->users_id]);
                $this->db->delete('users_roles', ['users_id' => $row->users_id]);
            }
            if ($Siswa > 0) {
                $data = [
                    'title' => 'Successfully',
                    'msg' => 'Berhasil menghapus ' . count($id_Siswa) . ' data',
                    'status' => 'success'
                ];
            } else {
                $data = [
    ...
    private function sessionSiswa($status, $title, $msg, $footer = null)
    {
    ...
    public function server_Siswa()
    {
        ...
        foreach ($list as $field) {
            ...
            <input id="checkbox_item' . $no . '" type="checkbox" class="check_item_Siswa" value="' . $field->id_siswa . '">
            ...
            if ($this->update) {
                $this->update = '<a href="' . base_url('Admin/Siswa/edit/' . $field->id_siswa) . '" class="btn btn-success shadow-sm"> <i class="fas fa-pencil-alt"></i></a>';
            }
            if ($this->delete) {
                $this->delete = '<a href="' . base_url('Admin/Siswa/delete/' . $field->id_siswa) . '" class="btn btn-danger delete_Siswa shadow-sm" data-id
                ="' . $field->id_siswa
                    . '"> <i class="fas fa-trash-alt"></i></a>';
            }
            if ($this->read) {
                $this->read = '<a href="' . base_url('Admin/Siswa/detail/' . $field->id_siswa) . '" class="btn btn-primary shadow-sm detailSiswa" data-id="' . $field->id_siswa . '"  data-toggle="modal" data-target="#modalSiswaDetail"> <i class="fas fa-eye"></i></a>';
            }
            $row[] = '
    ...
    public function export()
    {
        ...
        if ($get == null) {
            $Siswa = $this->Siswa_model->joinAllUsers()->result();
        } else {
            $id = explode(',', $get);
            foreach ($id as $result) {
                $row[] = $this->Siswa_model->joinAllUsers($result)->row();
            }
            $Siswa = $row;
        }
        ...
        foreach ($Siswa as $result) {
            $spreadsheet->setActiveSheetIndex(0)
                ...
                ->setCellValue('B' . $kolom, $result->nama_siswa)
    ...
    public function importData()
    {
        ...
        if (isset($_FILES['file']['name']) && in_array($_FILES['file']['type'], $file_mimes)) {
            ...
            for ($i = 1; $i < count($sheetData); $i++) {
                ...
                if ($cek != null) {
                    ...
                    if ($cekUsername->num_rows() > 0) {
                        sweetAlert2('error', 'Failed', 'Username ' . $username . ' sudah digunakan', null, 'Admin/Siswa');
                        exit();
                    }
                    $data_users = [
                        'username' => $username,
            ...
            if ($insert > 0) {
                sweetAlert2('success', 'Successfully', 'Berhasil ' . $count . ' import data Siswa', null, 'Admin/Siswa');
            } else {
                sweetAlert2('error', 'Failed', 'Gagal import data Siswa', null, 'Admin/Siswa');
            }
        }
    }
    public function detail()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            $row = $this->Siswa_model->get($id)->row();
            ...
            if ($row->keterangan == 'Lengkap') {
                ...
                <div class="table-responsive">
                    <table class="table table-striped">
                        ...
                        <tr>
                            <td colspan="3" class="text-center"><img src="' . base_url('vendor/img/siswa/' . $row->gambar) . '" class="img-fluid img-thumbnail w-50" alt="Gambar Siswa"></td>
                        </tr>
                    </table>
