...
class Guru extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $roles = check_roles_crud('Admin/Guru', $my_roles);
        ...
        $this->load->model(['Guru/Guru_model', 'Guru/Server_Guru_model', 'Auth/Users_model']);
    }
    public function index()
    { // add breadcrumbs
        $this->breadcrumbs->push('Home', 'Admin/Home');
        $this->breadcrumbs->push('Guru', 'Admin/Guru');
        // output
        $data['title'] = 'Mangement Guru';
        $data['breadcrumb'] = $this->breadcrumbs->show();
        $this->template->breadcrumbs($data);
        $data['create'] = $this->create;
        $data['delete'] = $this->delete;
        // add breadcrumbs
        $this->template->admin('admin/guru/main', $data);
    }
    public function add()
    {
        if ($this->create != true) {
        ...
        $this->breadcrumbs->push('Home', 'Admin/Home');
        $this->breadcrumbs->push('Guru', 'Admin/Guru');
        $this->breadcrumbs->push('Form Data', 'Admin/Guru/add');
        // output
        ...
        $this->template->admin('admin/guru/form_data', $data);
    }
        ...
        if (isset($_POST['add'])) {
            if ($this->form_validation->run() == false) {
                ...
                if ($keterangan != 'Lengkap') {
                    ...
                    $data = [
                        'status_ttq' => htmlspecialchars($this->input->post('status_ttq', true)),
                        'nomor_induk' => htmlspecialchars($this->input->post('nomor_induk', true)),
                ...
                if ($insert > 0) {
                    sweetAlert2('success', 'Successfully', 'Berhasil tambah data Guru', null, 'Admin/Guru');
                } else {
                    sweetAlert2('success', 'Successfully', 'Berhasil tambah data Guru', null, 'Admin/Guru');
                }
            }
            ...
            if ($this->form_validation->run() == false) {
                ...
                $get = $this->Guru_model->get($id)->row();
                ...
                $update = $this->Guru_model->update($data, $id);
                if ($update > 0 || $updateUsers > 0) {
                    sweetAlert2('success', 'Successfully', 'Berhasil update data Guru', null, 'Admin/Guru');
                } else {
                    sweetAlert2('error', 'Failed', 'Gagal update data Guru', null, 'Admin/Guru');
                }
            }
        } else {
            show_404();
    ...
    public function edit($id)
    {
        ...
        if (!isset($id)) {
            ...
            $this->breadcrumbs->push('Home', 'Admin/Home');
            $this->breadcrumbs->push('Guru', 'Admin/Guru');
            $this->breadcrumbs->push('Form Guru', 'Admin/Guru/edit/' . $id);
            // output
            $data['breadcrumb'] = $this->breadcrumbs->show();
            $data['title'] = 'Form Guru';
            $this->template->breadcrumbs($data);
    ...
    private function uploadGambar($jenis_kelamin = '')
    {
        $gambar = $_FILES['gambar']['name'];
        if ($gambar != null) {
            $config['upload_path'] = './vendor/img/guru';
            ...
            if (!$this->upload->do_upload('gambar')) {
                ...
                $config['source_image'] = './vendor/img/guru/' . $gambar['file_name'];
                $config['create_thumb'] = FALSE;
                $config['maintain_ratio'] = FALSE;
                $config['quality'] = '50%';
                $config['width'] = 600;
                $config['height'] = 600;
                $config['new_image'] = './vendor/img/guru/' . $gambar['file_name'];
    ...
    {
        $select_image = $this->Guru_model->get($id)->row();
        if ($select_image != null) {
            if ($select_image->gambar != null) {
                if ($select_image->gambar != 'default_female.png' && $select_image->gambar != 'default_male.jpg') {
                    ...
                    return array_map('unlink', glob(FCPATH . "vendor/img/guru/" . $filename . '.*'));
                }
            } else {
                return null;
            }
    ...
    public function delete()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            $id_Guru = htmlspecialchars($this->input->post('id', true));
            $this->removeImage($id_Guru);
            $row = $this->Guru_model->get($id_Guru)->row();
            $Guru = $this->Guru_model->delete($id_Guru);
    ...
    public function deleteAllGuru()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            $id_Guru = $this->input->post('data_item');
            foreach ($id_Guru as $index => $result) {
                $this->removeImage($result);
                $row = $this->Guru_model->get($result)->row();
                $Guru = $this->Guru_model->delete($result);
                $this->db->delete('users', ['id_users' => $row->users_id]);
                $users = $this->db->affected_rows();
                $this->db->delete('users_roles', ['users_id' => $row->users_id]);
            }
            if ($Guru > 0 || $users > 0) {
                $data = [
                    'title' => 'Successfully',
                    'msg' => 'Berhasil menghapus ' . count($id_Guru) . ' data',
                    'status' => 'success'
                ];
            } else {
                $data = [
    ...
    private function sessionGuru($status, $title, $msg, $footer = null)
    {
    ...
    public function server_Guru()
    {
        $list = $this->Server_Guru_model->get_datatables();
        $data = array();
        $no = $_GET['start'];
        foreach ($list as $field) {
            ...
            <input id="checkbox_item' . $no . '" type="checkbox" class="check_item_Guru" value="' . $field->id_guru . '">
            ...
            if ($this->update) {
                $update = '<a href="' . base_url('Admin/Guru/edit/' . $field->id_guru) . '" class="btn btn-success shadow-sm"> <i class="fas fa-pencil-alt"></i></a>';
            }
            if ($this->delete) {
                $delete = '<a href="' . base_url('Admin/Guru/delete/' . $field->id_guru) . '" class="btn btn-danger delete_Guru shadow-sm" data-id
                ="' . $field->id_guru
                    . '"> <i class="fas fa-trash-alt"></i></a>';
            }
            if ($this->read) {
                $read = '<a href="' . base_url('Admin/Guru/detail/' . $field->id_guru) . '" class="btn btn-primary shadow-sm detailGuru" data-id="' . $field->id_guru . '"  data-toggle="modal" data-target="#modalGuruDetail"> <i class="fas fa-eye"></i></a>';
            }
            $row[] = '
        ...
        $output = array(
            "draw" => $_GET['draw'],
            "recordsTotal" => $this->Server_Guru_model->count_all(),
            "recordsFiltered" => $this->Server_Guru_model->count_filtered(),
            "data" => $data,
        );
    ...
    public function export()
    {
        ...
        if ($get == null) {
            $Guru = $this->Guru_model->joinAllUsers()->result();
        } else {
            $id = explode(',', $get);
            foreach ($id as $result) {
                $row[] = $this->Guru_model->joinAllUsers($result)->row();
            }
            $Guru = $row;
        }
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('N1', 'Status Guru TTQ');
        ...
        foreach ($Guru as $result) {
            $spreadsheet->setActiveSheetIndex(0)
                ...
                ->setCellValue('B' . $kolom, $result->nama_guru)
                ...
                ->setCellValue('N' . $kolom, $result->status_ttq);
        ...
        $spreadsheet->getActiveSheet()->getStyle('A1:N1')->applyFromArray($styleArray_title);
        ...
        $kolom = $kolom - 1;
        $spreadsheet->getActiveSheet()->getStyle('A2:N' . $kolom)->applyFromArray($styleArrayColumn);
        $spreadsheet->getActiveSheet()->getStyle('A2:N' . $kolom)
    ...
    public function importData()
    {
        ...
        if (isset($_FILES['file']['name']) && in_array($_FILES['file']['type'], $file_mimes)) {
            ...
            for ($i = 1; $i < count($sheetData); $i++) {
                ...
                if ($cek != null) {
                    ...
                    if ($cekUsername->num_rows() > 0) {
                        sweetAlert2('error', 'Failed', 'Username ' . $username . ' sudah digunakan', null, 'Admin/Guru');
                        exit();
                    }
                    $data_users = [
                        'username' => $username,
            ...
            if ($insert > 0) {
                sweetAlert2('success', 'Successfully', 'Berhasil ' . $count . ' import data Guru', null, 'Admin/Guru');
            } else {
                sweetAlert2('error', 'Failed', 'Gagal import data Guru', null, 'Admin/Guru');
            }
        }
    }
    public function detail()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            $row = $this->Guru_model->get($id)->row();
            ...
            $status_guru = $row->status_ttq == "y" ? "<span class='badge badge-info'>Dosen P.A</span>" : "<span class='badge badge-info'>Dosen Mentoring</span>";
            if ($row->keterangan == 'Lengkap') {
                ...
                <div class="table-responsive">
                    <table class="table table-striped">
                        ...
                        <tr>
                            <td colspan="3" class="text-center"><img src="' . base_url('vendor/img/guru/' . $row->gambar) . '" class="img-fluid img-thumbnail w-50" alt="Gambar Guru"></td>
                        </t>
                    </table>
